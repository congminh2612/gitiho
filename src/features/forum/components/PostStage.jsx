...
const PostStage = ({ status }) => {
    ...
    const { data: posts, isSuccess, isLoading } = useQuery({
        queryKey: ['posts', status, accountId],
        queryFn: status === 'all' ? getAllPost : () => getPostByStatus(status, accountId)
    })
                            ...
                            <div key={post.postId} className='bg-white shadow-md p-8'>
                                <div className='flex justify-between'>
                                    <div className='flex'>
                                        ...
                                        <p className='text-base ml-2 text-primary'>{post.postText}</p>
                                    </div>
                                ...
                                <div className='flex items-center space-x-4 mt-6'>
                                    <div className='relative'>
                                        {isLiked ?
                                            <GoHeartFill size='24' color='red' className='cursor-pointer hover:text-red-200' onClick={() => mutationUnlike.mutate(post.postId)} />
                                            :
                                            <GoHeart size='24' className='cursor-pointer' onClick={() => mutationLike.mutate(post.postId)} />
                                        }
                                        <div className='absolute size-5 bg-primary flex justify-center items-center text-white text-sm rounded-full -top-3 -right-3'>{post.postlikes.length}</div>
                                    </div>

                                    <div className='relative cursor-pointer' onClick={() => handleOpenModal(post.postId, post.postFile)} >
                                        <FaRegComment size='24' />
                                        <div className='absolute size-5 bg-primary flex justify-center items-center text-white text-sm rounded-full -top-3 -right-3'>{post.countComment}</div>
                                    </div>
                                </div>
                                <div>
