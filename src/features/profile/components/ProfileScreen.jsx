...
import { handleValidationChangePassword, handleValidationUpdateUser } from '../../../utils/validate'
import { getAllPhone } from '../../auth/services/getAllPhone';
import '../../../assets/ProfileStyle.css';
import '../../../assets/InforUserStyle.css';
import { v4 } from "uuid";
import { ChangePassowrdService, GetInforByEmailService, GetPhoneWithoutThisPhonedService, UpdateUserService } from '../service/profile'
import { updateUser } from '../../../admin/services/updateUser'
...
const ProfileScreen = () => {
  ...
  const handleViewUser = async () => {
    ...
    if (account) {
      setEditData({
        ...
        editSchoolName: account.schoolName,
      });
      setShowUpdate(true);
    }
  };
  ...
  const handleUpdate = async () => {
    ...
    var result = await GetPhoneWithoutThisPhonedService(account.phone);
    ...
    if (Object.keys(errors).length === 0) {
      ...
      if (result) {
        ...
        setErrors([]);
        const response = await GetInforByEmailService(account.email);
        if (response) {
          dispatch(loginSuccess({ account: response.result.user }))
  ...
  const handleChangePassword = async () => {
    ...
    if (Object.keys(errors).length === 0) {
      const result = await ChangePassowrdService(account.accountId, newPassword.inputNewPassword);
      if (result) {
  ...
  return (
    <div className='container mx-auto pl-[100px] pt-10'>
      {contextHolder}
            ...
            <div className='sc-eCVOBu ddbBzc'>
              <div className='sc-ibAmJv cHgnBy'>
                <div className='sc-dOFTbv iBYITo'>
                  <img
                    src='/public/Image/email-part-2-svgrepo-com.svg'
                    alt=''
      ...
      <Modal
        title={
          <h2 style={{ color: '#2484ba', fontSize: '20px', fontWeight: 'bold', width: '60%' }}>
            Thay đổi mật khẩu
          </h2>
        }
