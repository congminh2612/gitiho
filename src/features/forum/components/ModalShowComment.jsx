...
const ModalShowComment = () => {
    ...
    const { data: comments, isSuccess } = useQuery({ queryKey: ['comment', postId], queryFn: () => getCommentByPost(postId) })
        ...
        <ModalBase className='w-[800px]' handleClose={() => dispatch(closeModal())}>
            <div className='p-6'>
                ...
                <div className='space-y-6 mt-10 h-[300px] overflow-auto'>
                    {isSuccess && comments.map((cmt) => {
                        return (
                            <div key={cmt.postCommentId} className='flex items-center space-x-6 mr-6'>
                                <div className="size-10 rounded-full">
                                    <img src={cmt.avatar} alt="" width={50} className='rounded-full' />
                                </div>
