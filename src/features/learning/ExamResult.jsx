  ...
  return (
    <div className="exam-finish p-4">
      ...
      {testDetail.map((item) => (
        <div
          key={item.id}
          className="exam-finish-result bg-white shadow-md rounded-lg p-6 mb-6"
        >
          <h5 className="text-xl font-semibold mb-2">{item.topicName}</h5>
          <hr className="mb-4" />
          <p className="mb-2">
            Môn thi: <span className="font-medium">{item.subjectName}</span>
          </p>
          <p className="mb-2">
            Số câu hỏi:{' '}
            <span className="font-medium">{item.totalQuestion} câu</span>
          </p>
          <p className="mb-2">
            Thời gian: <span className="font-medium">{item.duration} phút</span>
          </p>
          <p className="mb-2">
            Số câu đúng:{' '}
            <span
              className={`${item.score < 5 ? 'text-red-500' : ''} font-medium`}
            >
          ...
          <div className="exam-finish-result-button flex mt-4">
            <div
              className="exam-finish-result-button-other bg-gray-200 hover:bg-gray-300 text-gray-800 py-2 px-4 rounded-lg mr-2 cursor-pointer"
              onClick={() => navigate('/')}
            >
              Trang chủ
            </div>
