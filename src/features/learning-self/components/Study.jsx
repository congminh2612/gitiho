    ...
    <div className="container mx-auto px-[100px]">
      ...
      <ModalBase>
        {score !== null && (
          <>
            <p>
              Chúc mừng bạn đã đúng {score} trên {questions.data.length} câu
            </p>
            <p>
              Số điểm bạn đạt được:{' '}
              <span className="text-lg ml-1 text-red-500">
                {parseInt((10 * score) / questions.data.length)}
              </span>
            </p>
          </>
        )}
        <div className="flex items-center justify-between mt-4">
          <BaseButton
            ...
            className="px-4 py-[5px] bg-red-500 text-white  rounded-xl"
          />
          <BaseButton
            handleClick={handleShowAnswers}
            title="Xem đáp án"
            className=" px-4 py-[5px] bg-slate-600 rounded-xl"
          />
        </div>
      </ModalBase>
    </div>
