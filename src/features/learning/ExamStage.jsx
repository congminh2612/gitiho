        ...
        <div className="p-4">
            <div className="flex justify-between items-center mb-4">
                <div className="flex items-center">
                    <img src={''} alt="Timer" />
                    <h4 className="ml-2 text-xl font-bold">{minutes}:{seconds < 10 ? `0${seconds}` : seconds}</h4>
                </div>
            </div>
            <div className="flex">
                <div className="w-1/4 p-4 border-r border-gray-300">
                    ...
                    <div>
                        <button className="w-full py-2 bg-blue-500 text-white font-bold rounded" onClick={showConfirm}>Nộp bài</button>
                    </div>
                </div>
                    ...
                    {questions.map((item, index) => (
                        <div key={index} className="mb-6" id={item.questionId}>
                            <div className="mb-2 flex items-center">
                                ...
                                <div dangerouslySetInnerHTML={{ __html: item.questionContext }}></div>
                                {item.image && <img src={item.image} alt="Question" className="ml-2" />}
                            </div>
                                ...
                                {['A', 'B', 'C', 'D'].map((option, idx) => (
                                    <div key={idx} className="flex items-center">
                                        ...
                                        <label
                                            ...
                                            className="ml-2"
                                            dangerouslySetInnerHTML={{ __html: item[`option${option}`] }}
                                        ></label>
                                    </div>
                                ))}
                            </div>
